...
parser.add_argument('tgt_dataset', type=str, choices=["gta", "city", "test", "ir", "city16", "Taipei", "Tokyo", "Roma", "Rio"])
...
for index, (imgs, _, paths) in tqdm(enumerate(target_loader)):
    ...
    with open(info_json_fn) as f:
        ...
        pairs = [pair for pair in city_info_dic["label2train"]]
        pairs = pairs[::-1]
        for p in pairs:
            prediction[np.where(prediction == p[1])] = p[0]
    #####################
    img = Image.fromarray(np.uint8(pred.numpy()))
    img2 = Image.fromarray(np.uint8(prediction))
    img2 = img2.resize(test_img_shape, Image.NEAREST)
    
    label_outdir = os.path.join(base_outdir, "label")
    if index == 0:
        print ("pred label dir: %s" % label_outdir)
    mkdir_if_not_exist(label_outdir)
    label_fn = os.path.join(label_outdir, path.split('/')[-1]).replace('jpg','png')
    img2.save(label_fn)
    ...
    vis_fn = os.path.join(vis_outdir, path.split('/')[-1]).replace('jpg','png')
    img.save(vis_fn)
