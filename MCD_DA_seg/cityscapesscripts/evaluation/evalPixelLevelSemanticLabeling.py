...
parser.add_argument('--gt', type=str, required=True, help='Groundtruth label')
parser.add_argument('--pd', type=str, required=True, help='predict label')
...
def getPrediction( args, groundTruthFile ):
    # determine the prediction path, if the method is first called/ groundTruthFile
...
if '--city-val' in args_.pd:
    args.groundTruthSearch  = os.path.join( args_.gt +'/*/*_gtFine_labelIds.png' )
...
def main(argv):
    ...
    if (len(argv) > 4):
        for arg in argv:
            ...
            if ("seg" in arg or "groundtruth" in arg):
