...
class FCN8VGG:
    ...
    def build(self, batch_size, input_w=1024, input_h=512, train=False, num_classes=19, city='Taipei', 
        ...
        self.rgb = tf.placeholder(tf.float32, shape = [batch_size, input_h, input_w, 3], name = 'rgb')
        ...
        self.static_priors = tf.placeholder(tf.float32, shape = [int(batch_size/2), 256, 512, 1], name = 'static_priors')
        static_prior = tf.squeeze(self.static_priors, [3])
        ...
        if train:
            ...
            # the shape of task_labels is batch_size * height * width
        ...
        if random_init_adnn and train:
            ...
            with tf.variable_scope('class-specific_alignment'):
                ...
                static_prior = zero_gradient(static_prior)
                static_obj_region = tf.cast(tf.equal(static_prior, 100), tf.float32)
                
                # here is the static-prior comes into play
                # for the static-obj region, suppress the probabilities of all moving-object classes
                # for cityscapes dataset, class index i >=11 belongs to moving objects
                
                psuedo_label_static_mask = tf.ones([psuedo_labels.get_shape()[0].value, 
                                                    psuedo_labels.get_shape()[1].value, 
                                                    psuedo_labels.get_shape()[2].value, 
                                                    11], tf.float32)
                
                psuedo_label_mov_mask = tf.tile(tf.expand_dims(1-static_obj_region, 3), [1,1,1,8])
                psuedo_label_mask = tf.concat([psuedo_label_static_mask, psuedo_label_mov_mask], 3)

                suppressed_psuedo_labels = tf.multiply(psuedo_labels, psuedo_label_mask)
                normalized_psuedo_labels = tf.transpose(tf.div(tf.transpose(psuedo_labels), 
                                                               tf.transpose(tf.reduce_sum(psuedo_labels, [3]))))
                ...
                for i in range(num_classes):
                    ...
                    self.CA_adnn3s.append(self._score_layer(self.CA_adnn2s[i], "CA%d_adnn3" % i))
                    ...
                    cw_psuedo_labels = tf.transpose(tf.gather(tf.transpose(normalized_psuedo_labels), i))
